...
const TripRow = memo(({
    ...
    const [localId, setLocalId] = useState(trip.id || '');
    const isIdDirty = localId !== (trip.id || '');
    ...
    return (
        <tr className="hover:bg-primary-light/50 transition-colors animate-fade-in group border-b border-primary-light/20">
                    ...
                    {isIdDirty && (
                        <div className="flex gap-1 animate-in fade-in slide-in-from-top-1">
                            <button
                                ...
                                disabled={saving}
                                className="flex-1 bg-emerald-500 text-white p-1 rounded-lg hover:bg-emerald-600 transition-colors flex items-center justify-center gap-1 text-[10px] font-black"
                            >
                            ...
                            <button
                                ...
                                disabled={saving}
                                className="bg-slate-200 text-slate-600 p-1 rounded-lg hover:bg-slate-300 transition-colors px-2"
                            >
            ...
            <td className="p-4 text-right">
                <button
                    onClick={onDownload}
                    disabled={isIdDirty || saving}
                    title={isIdDirty ? "Save changes to enable download" : "Download PDF"}
                    className={`p-3 rounded-2xl transition-all shadow-sm border ${isIdDirty ? 'bg-slate-100 text-slate-300 border-slate-200 cursor-not-allowed' : 'text-primary hover:bg-primary hover:text-white bg-white border-primary-light active:scale-95'}`}
                >
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] text-amber-200/40 font-black tracking-[0.3em] uppercase hidden sm:flex items-center gap-2 mt-1">
                                ...
                                Trip Details Control Panel
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-3 sm:gap-4">
                        <div className="hidden lg:flex items-center gap-2 text-[10px] font-bold text-white bg-indigo-500/20 p-2 rounded-full border border-indigo-400/20 glass">
